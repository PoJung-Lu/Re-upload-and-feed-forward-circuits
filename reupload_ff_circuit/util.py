...
def initialize_data(
    problem, n_training=None, n_test=None, seed_num=0, enc_dim=3, **kwargs
):
...
def initialize_params(
    enc_dim, n_qubits, n_feedforward, n_reupload, n_rot, num_class, seed_num=40
):  # num_class= number of class to be classified for one qibit
    """
    ...
    p_s = (
        jax.random.normal(
            key_s, (n_feedforward, n_reupload, n_qubits, enc_dim), dtype=jnp.float64
        )
        * scale_s
    )
    p_c = (
        jax.random.normal(
            key_c, (n_feedforward, n_reupload, n_qubits, n_rot, 3), dtype=jnp.float64
        )
        * scale_c
    )
...
def plot_loss_history(l_h, val_l_h=[], setting=None, fig_name=""):
    ...
    plt.plot(
        range(len(l_h)), l_h, color=(255 / 255, 100 / 255, 100 / 255), label="Loss"
    )
    if val_l_h:
