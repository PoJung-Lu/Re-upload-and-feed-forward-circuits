...
def model_noise(backend, fake_backend):
    ...
    if fake_backend:  # deprecated
        ...
        logger.warning(
            "Fake backend deprecated, falling back to NoiseModel from physical backend."
        )
...
def device(
    num_qubits, noise=False, real_device=False, backend_name=None
):  # noise_model=None
    if noise:
        ...
        if real_device:
            ...
            dev = qml.device(
                ...
                wires=num_qubits,
                backend=backend,
            )  # initial_layout=[1,2], start_session=True)
        else:
...
class qcircuit:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if enc_dim < 1 or n_qubits < 1 or n_layers < 1:
            raise ValueError(
                f"Circuit dimensions must be positive integers: "
                f"enc_dim={enc_dim}, n_qubits={n_qubits}, n_layers={n_layers}"
            )
        if n_reupload < 1 or n_rot < 1:
            raise ValueError(
                f"Reupload and rotation counts must be positive: "
        ...
        if n_qubits > 10 and not noise:
            ...
            warnings.warn(
                f"Large qubit count ({n_qubits}) may cause memory issues with simulator. "
    ...
    def reshape_input(
        self, x_in
    ):  #'''May cause some prob ex. dim_in=4, enc_dim=3, q=5 e.g. not all enc_dim is filled. In this case, the rest of gates are set to 0 '''
        ...
        if remain_dim := enc_dim * n_qubits - len(
            x_in
        ):  # make x_in be integer multiple of enc_dim
            x_in = jnp.vstack((jnp.zeros((remain_dim, self.N)), x_in))
        x_in = x_in.reshape(
            len(x_in) // enc_dim, enc_dim, self.N
        )  # (least_q,enc_dim,N) ## each qubit have enc_dim input dims
        return x_in
            ...
            for e_i in range(self.enc_dim):
                if even := (e_i + 1) % 2:  # Rx(x[0]),Rx(x[2])...
                    ...
                    if e_i >= len(x[0]):
                        ...
                        logger.warning(
                            "Number of encoding features is more than enc_dim."
                        )
                else:  # Ry(x[1]),Ry(x[3])...
    ...
    def circuit(self, p, x, rot):
        ...
        x1 = jnp.tile(x, (self.enc_dim, 1)).reshape(
            self.n_qubits, self.enc_dim
        )  # (q)->(enc,q)->(q,enc)
    ...
    def j_circuit_end(self, p, x, y, n_layers, rot):
        ...
        if n_layers == 1:
            return jax.vmap(self.circuit_end, (None, 2, None, None, None))(
                p, x, y, n_layers, rot
            )
        else:
            return jax.vmap(self.circuit_end, (None, 1, None, None, None))(
                p, x, y, n_layers, rot
            )
    ...
    def qc_nq(self, params, x, y):
        ...
        self.least_q = math.ceil(
            self.dim_x / self.enc_dim
        )  # least qubit # for encoding
        if self.least_q > self.n_qubits:
            raise ValueError("Number of qubits not enough for encoding features.")
        ...
        if self._v_enc_circuit is None:
            ...
            self._v_circuit_end_single = jax.vmap(
                self.circuit_end, (None, 2, None, None, None)
            )
            self._v_circuit_end_multi = jax.vmap(
                self.circuit_end, (None, 1, None, None, None)
            )

        v_enc_circuit = self._v_enc_circuit
        v_circuit = self._v_circuit
        v_circuit_end = (
            self._v_circuit_end_single if n_layers == 1 else self._v_circuit_end_multi
        )
    ...
    def jqc_nq(self, params, x, y):
        ...
        for f_i in range(self.n_layers - 1):
            p = {i: j[f_i] for i, j in params.items()}
            ocs = (
                self.j_enc_circuit(p, x, rot) if f_i == 0 else self.j_circuit(p, x, rot)
            )
            x = jnp.array(ocs)
        p = {i: j[self.n_layers - 1] for i, j in params.items()}
...
def test(params, x, y, *args, **kwargs):
    ...
    def cost(params, x, y, *setting, **kwargs):
        ...
        # qc = kwargs['qc']
        tot_fidelities = jnp.array(
            [fidel_function(params, x_in, dm, *setting, **kwargs) for dm in dm_labels]
        ).T  # shape:(N,#qubit,#class)
        num_qubits = setting[1]

        tot_fidelities = tot_fidelities.reshape(len(y), num_qubits, num_class)

        tot_fids = (
            jax.vmap(jnp.kron, (0, 0), 0)(
                tot_fidelities[:, 0], tot_fidelities[:, 1]
            ).reshape(len(y), num_qubits, num_class)
            if shape == "bitwise"
            else tot_fidelities
        )

        Loss = jax.vmap(jloss, (None, 0, 0, None, None, None))(
            params["loss"], tot_fids, y, shape, num_class, Yc
        )
        Loss = jnp.sum(Loss) / len(y) / num_qubits
        predicted = (
            jax.vmap(jnp.argmax)(tot_fids)
            if shape == "bitwise"
            else jax.vmap(jnp.argmax)(tot_fids) % num_class
        )
    ...
    """grad go to 0 if u diff over fun of fun； 
       ...
       2022/12/26"""
    (loss, predicted), grad = jax.value_and_grad(cost, has_aux=True)(
        params, x, y, *args, **kwargs
    )
...
def jtest(params, x, y, *args, **kwargs):
    if kwargs["noise"]:
        ...
        # print(kwargs.keys())
        return jax.jit(test, static_argnums=(4), static_argnames=tuple(kwargs.keys()))(
            params, x, y, *args, **kwargs
        )
...
def scores(params, x_tr, y_tr, *args, x_te=None, y_te=None, report=False, **kwargs):
    ...
    if x_te is None and y_te is None:
        ...
        predicted_test, loss_test, grads_test = jtest(
            params, x_te, y_te, *args, **kwargs
        )
